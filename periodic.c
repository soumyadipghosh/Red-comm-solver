...
void inidat();
void update(int, int, int, int, int, int, long int, double, double*, double*, double*, double*);
...
/* First, find out my taskid and how many tasks are running */
   ...
   if (taskid == MASTER) {
      ...
      {
         //rows = (i <= extra) ? averow+1 : averow; //consider uniform grid division
         offset = offset + rows;
      ...
      do
      {
         ...
         {
            MPI_Issend((u+iz*NXPROB*NXPROB+offset*NXPROB), NXPROB, MPI_DOUBLE, left,
                     RTAG*1, MPI_COMM_WORLD, &req1);
            source = left;
            msgtype = LTAG;
            MPI_Recv((u+iz*NXPROB*NXPROB+(offset-1)*NXPROB), NXPROB, MPI_DOUBLE, source,
                      msgtype*1, MPI_COMM_WORLD, &status);
            for(iy = 0; iy < NXPROB; iy++)
            {
         ...
         {
            MPI_Issend((u+iz*NXPROB*NXPROB+(offset+rows-1)*NXPROB), NXPROB, MPI_DOUBLE, right, LTAG*1, MPI_COMM_WORLD, &req2);
            source = right;
            msgtype = RTAG;
            MPI_Recv((u+iz*NXPROB*NXPROB+(offset+rows)*NXPROB), NXPROB, MPI_DOUBLE, source, msgtype*1, MPI_COMM_WORLD, &status);
            for(iy = 0; iy < NXPROB; iy++)
            {
        ...
        update(start, end, NXPROB, pos_loc, neg_loc, NSTEP, wsteps, h, (u+iz*NXPROB*NXPROB), (u+(1-iz)*NXPROB*NXPROB), psource, residual);
     ...
     if(taskid != MASTER)
     {
         ...
         MPI_Send((u+iz*NXPROB*NXPROB+offset*NXPROB), rows*NXPROB, MPI_DOUBLE, MASTER, DONE,
                  MPI_COMM_WORLD);
 ...
 *  subroutine update
 ****************************************************************************/
void update(int start, int end, int ny, int pos_loc, int neg_loc, int NSTEP, long int wsteps, double h, double *u1, double *u2, double *psource, double *residual)
{
   int ix, iy;
   ...
   { 
      for (iy = 1; iy <= ny-2; iy++) 
      {
        ...
        *(u2+ix*ny+iy) = 0.25 * (*(u1+(ix+1)*ny+iy) + *(u1+(ix-1)*ny+iy) + *(u1+ix*ny+iy+1) + *(u1+ix*ny+iy-1) - h * h * *(psource + ix*ny + iy)); 

        if(wsteps % NSTEP == 0)
        {
            *(residual+ix*ny+iy) = (*(u1+(ix+1)*ny+iy) + *(u1+(ix-1)*ny+iy) + *(u1+ix*ny+iy+1) + *(u1+ix*ny+iy-1) - 4 * *(u1+ix*ny+iy))/(h * h) - *(psource + ix*ny + iy);
        }
