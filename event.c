...
void inidat();
void update(int, int, int, int, int, double, double*, double*, double*, double*);
...
int	taskid = 0,                               /* this task's unique id */
	...
	averow = 0,rows = 0,offset = 0,extra = 0, /* for sending rows of data */
...
double global_max_residual=0.0;
   ...
   MPI_Win_create(win_mem, 2*NXPROB*sizeof(double), sizeof(double),MPI_INFO_NULL,MPI_COMM_WORLD,&win);
   ...
   if (taskid == MASTER) {
      ...
      /* Initialize grid */
      printf("Grid size: X = %d Y = %d Threshold = %2.18f Period = %d\n",NXPROB,NXPROB,thres,NSTEP);
      printf("Initializing grid and writing initial.dat file...\n");
      inidat(NXPROB, NXPROB, psource, pos_loc, neg_loc); //for first block of 3rd dimension
      ...
      for (i=1; i < numtasks; i++)
      {
         ...
         MPI_Send((psource+offset*NXPROB), rows*NXPROB, MPI_DOUBLE, dest, BEGIN,
                  MPI_COMM_WORLD);
      }
    ...
    if(taskid != MASTER) //only other processors need to receive data
    {
         ...
         MPI_Recv((psource+offset*NXPROB), rows*NXPROB, MPI_DOUBLE, source, msgtype,
    ...
    */
      ...
      {         
         ...
         if (right != NONE)
         {
            for(iy = 0; iy < NXPROB; iy++)
            {
		curr_value = *(u+iz*NXPROB*NXPROB+(offset+rows-1)*NXPROB+iy);
            ...
            if(right_flag == 1)
            {
                ...
                MPI_Put((u+iz*NXPROB*NXPROB+(offset+rows-1)*NXPROB), NXPROB, MPI_DOUBLE, right, 0, NXPROB, MPI_DOUBLE, win); //to left ghost cells
                //for(iy = 0; iy < NXPROB; iy++) MPI_Put((u+iz*NXPROB*NXPROB+(offset+rows-1)*NXPROB+iy), 1, MPI_DOUBLE, right, iy, 1, MPI_DOUBLE, win);
		...
		{
		    printf("This is right msg from PE %d in iter %ld - grid point - %d\n",taskid,wsteps,iy);
		}
		*/
             }
         }
         ...
         if (left != NONE)
         {
	        ...
	        MPI_Put((u+iz*NXPROB*NXPROB+offset*NXPROB), NXPROB, MPI_DOUBLE, left, NXPROB, NXPROB, MPI_DOUBLE, win);
                                     //to right ghost cells
                //for(iy = 0; iy < NXPROB; iy++) MPI_Put((u+iz*NXPROB*NXPROB+offset*NXPROB+iy), 1, MPI_DOUBLE, left, (NXPROB+iy), 1, MPI_DOUBLE, win);
	        MPI_Win_unlock(left,win);
	        left_send_count++;
                /*
                else
                {
                    printf("This is left msg from PE %d in iter %ld - grid point - %d\n",taskid,wsteps,iy);
                }
                */
            }
         }
         
          //receiver side - this is not needed but we copy the shared memory values to the actual variables
          for(iy = 0; iy < NXPROB; iy++)
          {
             //copy to left ghost cells
             if(left != NONE)
             {
                *(u+iz*NXPROB*NXPROB+(offset-1)*NXPROB+iy) = *(win_mem+iy);
                *(u+(1-iz)*NXPROB*NXPROB+(offset-1)*NXPROB+iy) = *(u+iz*NXPROB*NXPROB+(offset-1)*NXPROB+iy);
             }

             //copy to right ghost cells
             if(right != NONE)
             {
                *(u+iz*NXPROB*NXPROB+(offset+rows)*NXPROB+iy) = *(win_mem+(NXPROB+iy));
                *(u+(1-iz)*NXPROB*NXPROB+(offset+rows)*NXPROB+iy) = *(u+iz*NXPROB*NXPROB+(offset+rows)*NXPROB+iy);
             }
          }

         //printf("This is proc %d in %d iteration: local max iter error: %2.5f, left flag-%d, right flag-%d\n",
         //                                               taskid,wsteps,local_max_iter_error,left_flag,right_flag);
         //if(taskid == 0) printf("In iteration %ld\n",wsteps);
         
         update(start, end, NXPROB, pos_loc, neg_loc, h, (u+iz*NXPROB*NXPROB), (u+(1-iz)*NXPROB*NXPROB), psource, residual); //first update values
                                                                 
         if(wsteps%NSTEP == 0)
         {
             local_avg_residual = calc_residual_sum(start, end, NXPROB, residual); //sum for this PE
             local_max_residual = calc_residual_max(start, end, NXPROB, residual); //max for this PE

         //MPI_Allreduce(&local_avg_residual, &global_avg_residual, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); //sum of error for entire grid
         //global_avg_residual = global_avg_residual/(NXPROB*NXPROB);

             MPI_Allreduce(&local_max_residual, &global_max_residual, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD); //sum of error for entire grid
          
        //if(taskid == MASTER) fprintf(fptr, "%d %f\n", wsteps, global_max_residual);
         }
      ...
      {         
          ...
          MPI_Send((u+iz*NXPROB*NXPROB+offset*NXPROB), rows*NXPROB, MPI_DOUBLE, MASTER, DONE, MPI_COMM_WORLD);
          MPI_Send((residual+offset*NXPROB), rows*NXPROB, MPI_DOUBLE, MASTER, DONE,
                  MPI_COMM_WORLD);      
      }
      ...
      if(taskid == MASTER)
      {
         ...
         for (i=1; i < numtasks; i++)
         {
           ...
           MPI_Recv(&offset, 1, MPI_INT, source, msgtype, MPI_COMM_WORLD,
                    &status);
           MPI_Recv(&rows, 1, MPI_INT, source, msgtype, MPI_COMM_WORLD, &status);
           MPI_Recv((u+0*NXPROB*NXPROB+offset*NXPROB), rows*NXPROB, MPI_DOUBLE, source,
 ...
 *  subroutine update
 ****************************************************************************/
void update(int start, int end, int ny, int pos_loc, int neg_loc, double h, double *u1, double *u2, double *psource, double *residual)
{
   int ix, iy;
   for (ix = start; ix <= end; ix++)
   { 
      for (iy = 1; iy <= ny-2; iy++) //by not calculating for boundary values, we impose boundary conditions
      {
        //if(ix == pos_loc && iy == pos_loc) continue; //dont calculate for positive dipole
        //if(ix == neg_loc && iy == neg_loc) continue; //dont calculate for negative dipole
        *(u2+ix*ny+iy) = 0.25 * (*(u1+(ix+1)*ny+iy) + *(u1+(ix-1)*ny+iy) + *(u1+ix*ny+iy+1) + 
                                                              *(u1+ix*ny+iy-1) - h * h * *(psource + ix*ny + iy)); 

        //if(wsteps % NSTEP == 0)
        //{
            *(residual+ix*ny+iy) = (*(u1+(ix+1)*ny+iy) + *(u1+(ix-1)*ny+iy) + *(u1+ix*ny+iy+1) + 
                                                    *(u1+ix*ny+iy-1) - 4 * *(u1+ix*ny+iy))/(h * h) - *(psource + ix*ny + iy);
        //}
        //if(*(residual+ix*ny+iy) != 0.0) printf("Non-zero residual - %f\n",*(residual+ix*ny+iy));
      }
   }
}
...
//for avg error (mat is both iteration error as well as ss error at any time step 
...
//for max error (mat is both iteration error as well as ss error at any time step 
